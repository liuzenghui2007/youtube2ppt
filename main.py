...
def main() -> None:
    ...
    parser.add_argument("-o", "--output-dir", default="", help="输出目录（默认用 config）")
    ...
    if args.preview:
        ...
        if not crop:
            ...
            sys.exit(1)
        run_crop(video_path, output_dir, crop, force=cfg.get("force_crop"))
        paths = run_preview_frames(video_path, output_dir, crop)
        print("预览已生成:", [str(p) for p in paths])

    if args.extract:
        video_full = output_dir / "video.mp4"
        video_cropped = output_dir / "video_cropped.mp4"
        ...
        if not video_cropped.is_file():
            run_crop(video_full, output_dir, crop, force=cfg.get("force_crop"))
        run_extract(
            output_dir,
            video_full,
